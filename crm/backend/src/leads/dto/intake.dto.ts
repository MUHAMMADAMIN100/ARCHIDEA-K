...
class ContactDto {
   ...
   *
   * Поле формы заполнено кодом страны с самого начала, и если посетитель его
   * не трогал, сюда приходит именно этот огрызок. Раньше проверка видела
   * непустую строку, браковала её — и ВСЯ заявка получала отказ. Человек
   * видел «не удалось отправить», хотя всё заполнил правильно: запасной
   * номер необязателен.
   *
   * Считаем поле пустым, когда цифр меньше семи: короче не бывает ни одного
   * международного номера, а код страны сам по себе телефоном не является.
   * Именно семь, а не «нет цифр после 992»: страну теперь выбирают любую,
   * и огрызок «+7» прежней проверкой уже не ловился.
   */
  @IsOptional()
  @Transform(({ value }) =>
