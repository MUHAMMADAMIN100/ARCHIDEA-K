...
class ContactDto {
   ...
   *
   * Поле формы заполнено кодом страны («+992 ») с самого начала, и если
   * посетитель его не трогал, сюда приходит именно этот огрызок. Раньше
   * проверка видела непустую строку, браковала её — и ВСЯ заявка получала
   * отказ «укажите номер из 9 цифр». Человек видел «не удалось отправить»,
   * хотя всё заполнил правильно: запасной номер необязателен.
   *
   * Поэтому значение без цифр (только код страны или пробелы) считаем
   * отсутствующим ещё до проверки.
   */
  @IsOptional()
  @Transform(({ value }) =>
    String(value ?? '').replace(/\D/g, '').replace(/^992/, '').length === 0
      ? undefined
      : value,
  )
  @IsString()
