...
/**
 * Срок жизни сессии — 5 лет, и он продлевается при каждом открытии CRM
 * (см. refreshSession и AuthController.me). Фактически вход постоянный:
 * залогинился один раз — и дальше всегда сразу своя страница.
 *
 * История решения. Сначала вход жил 12 часов — пароль каждый день. Потом
 * 90 дней с продлением — но сотрудника, не заходившего три месяца, всё же
 * выбрасывало. В августе 2026 владелец решил окончательно: «один раз
 * залогинились — сохранилось навсегда».
 *
 * Это не отменяет защиту: «Выйти» и «Выйти на всех устройствах» гасят
 * сессию мгновенно (sessionEpoch), а отключённый или удалённый сотрудник
 * теряет доступ при первой же проверке. Потерянный телефон закрывается
 * этими двумя рычагами — они и есть настоящая защита, а не срок куки.
 *
 * Честная оговорка: браузеры обрезают срок ЛЮБОЙ куки до 400 дней —
 * это их жёсткий потолок, замерено (просим 5 лет, Chrome ставит 400).
 * Продление при каждом открытии сбрасывает этот счётчик, поэтому на
 * практике вход всё равно постоянный: выбросит только того, кто не
 * открывал CRM больше 400 дней подряд.
 */
export const SESSION_DAYS = 5 * 365;
