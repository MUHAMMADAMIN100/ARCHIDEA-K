...
export class OrdersService {
  ...
  /**
   * Заказы для календаря: всё, что занимает хотя бы один день диапазона.
   *
   * Календарь раньше кормился доской воронки, а её закрытые колонки держат
   * только текущий месяц — прошлый месяц в календаре выглядел пустым, хотя
   * заказы за него были (за август виден один заказ из десятков). Здесь
   * отдаётся месяц ЦЕЛИКОМ, любые этапы, включая закрытые и отказы.
   *
   * День заказа — тот же, каким его рисует календарь: день уборки, без него
   * предпочтительная дата, без обеих — день оформления. Многодневная уборка
   * попадает в диапазон, если пересекается с ним хотя бы одним днём.
   */
  calendar(user: AuthUser, q: { from?: string; to?: string }) {
    // без диапазона — текущий месяц: календарь всегда просит конкретные дни,
    // это лишь страховка от запроса «отдай вообще всё»
    const сегодня = dayKey(new Date());
    const месяц = сегодня.slice(0, 7);
    const конецМесяца = new Date(
      Date.UTC(Number(месяц.slice(0, 4)), Number(месяц.slice(5, 7)), 0),
    ).getUTCDate();
    const дни = momentRange(
      q.from || `${месяц}-01`,
      q.to || `${месяц}-${String(конецМесяца).padStart(2, '0')}`,
    );
    return this.prisma.order.findMany({
      where: {
        ...this.scopeWhere(user),
        OR: [
          // уборка начинается внутри диапазона
          { scheduledDate: { gte: дни.gte, lte: дни.lte } },
          // многодневная началась раньше, но ещё не закончилась
          { scheduledDate: { lte: дни.lte }, scheduledEndDate: { gte: дни.gte } },
          { scheduledDate: null, preferredDate: { gte: дни.gte, lte: дни.lte } },
          {
            scheduledDate: null,
            preferredDate: null,
            createdAt: { gte: дни.gte, lte: дни.lte },
          },
        ],
      },
      include: orderInclude,
