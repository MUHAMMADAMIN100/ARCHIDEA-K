 ...
 */
/*
 * Метка в тексте шаблона — КРУГЛЫЕ скобки: «(Имя клиента)». Решение
 * владельца: двойные фигурные пугали людей, круглые привычны любому.
 *
 * Фигурные тоже разбираем — в старых шаблонах они остались, и ломать их
 * нельзя. Длину ограничиваем: незакрытая скобка в начале письма иначе
 * «съела» бы весь текст до следующей в конце.
 */
const PLACEHOLDER_CURLY = /\{\{\s*([^{}\n]{1,60}?)\s*\}\}/g;
const PLACEHOLDER_ROUND = /\(\s*([^()\n]{1,60}?)\s*\)/g;

/**
 * Заменяет «(Имя клиента)» — и старое «{{Имя клиента}}», «{{client}}» — на
 * значение из values.
 *
 * ГЛАВНОЕ ПРАВИЛО круглых скобок: незнакомое в скобках остаётся КАК
 * НАПИСАНО. Круглые скобки нужны и в обычной речи — «уборка (по
 * договорённости)», «санузел (2 шт.)». Стирать такой текст нельзя: клиент
 * получил бы письмо с дырой на месте пояснения. Поэтому подставляем только
 * известные названия, всё остальное не трогаем.
 *
 * Пустое значение известной метки даёт пустую строку — так было и раньше:
 * скидки нет, значит в письме на её месте ничего.
 */
export function renderPlaceholders(
  ...
  if (!text) return '';
  const substitute = (match: string, token: string): string => {
    const key = resolveKey(token);
    // не наша метка — возвращаем ровно то, что было написано
    if (!key) return match;
    return values[key] ?? '';
  };
  return text
    .replace(PLACEHOLDER_CURLY, substitute)
    .replace(PLACEHOLDER_ROUND, substitute);
}
