...
export class ProposalsService {
  ...
  async send(user: AuthUser, id: string, dto: SendProposalDto) {
    ...
    const updated = await this.prisma.$transaction(async (tx) => {
      ...
      if (sent.orderId) {
        /*
         * Двигаем заказ на этап «Коммерческое предложение» ТОЛЬКО вперёд.
         *
         * Раньше стадия перезаписывалась безусловно: КП, отправленное по уже
         * подтверждённому или оплаченному заказу, откатывало его назад в обход
         * штатной смены этапа — не снималась дата закрытия, не пересчитывалась
         * метка повторного клиента и, главное, в книге доходов оставался
         * автодоход по заказу, который формально уже не оплачен.
         */
