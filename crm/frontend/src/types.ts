...
/**
 * Видит ли пользователь СВОДНЫЕ данные компании: аналитику по менеджерам,
 * общую ленту журнала изменений, чужие профили.
 *
 * Не путать с доступом к РАЗДЕЛАМ: менеджер работает во всех разделах, кроме
 * финансовых. Права на разделы — это userManagesOps / userManagesCatalogs
 * ниже (на бэкенде — permissions.ts). И не путать с клиентской базой: заказы
 * и клиенты общие для всех, это userSeesWholeBase.
 */
 ...
 * заводит новый, двигает по воронке и может передать ответственного коллеге.
 * Раньше каждый видел только своих, и стоило человеку заболеть — его сделки
 * замирали: коллега их просто не находил.
 *
 * Отдельно от userSeesAll намеренно: сводные данные компании (аналитика,
 * общая история изменений, профили) остались у руководства, а личные списки —
 * задачи и напоминания — у каждого свои. То же правило на сервере:
 * seesWholeBase в common/decorators/current-user.decorator.ts.
 */
export function userSeesWholeBase(u?: AuthUser | null): boolean {
  return !!u;
}
...
export interface Analytics {
  ...
  revenue?: {
    ...
    period: number;
    /**
     * Расходы из книги за период БЕЗ зарплаты и премий (плитка «Все расходы»).
     * Зарплата и премии — отдельно, в payroll.staffPay; две плитки не
     * пересекаются и в сумме дают всю книгу (expensesTotal).
     */
    expenses: number;
    /** вся книга за период — для сверки двух плиток */
    expensesTotal?: number;
    /** чистый доход: выручка − ЗП клинеров − вся книга */
    net: number;
  };
...
export interface AnalyticsFull extends Omit<Analytics, 'revenue'> {
  ...
  revenue?: {
    ...
    period: number;
    /** расходы книги без зарплаты и премий (плитка «Все расходы») */
    expenses: number;
    /** вся книга за период: «ЗП и премии» + «Все расходы» */
    expensesTotal?: number;
    /** чистый доход: выручка − ЗП клинеров − вся книга */
    net: number;
  };
