...
function renderSample(text: string): string {
  const sub = (match: string, token: string): string => {
    const value = SAMPLE_BY_NAME[token.trim().toLowerCase().replace(/\s+/g, ' ')];
    /*
     * Незнакомое в скобках оставляем как есть — ровно так поступит и сервер.
     * Круглые скобки нужны в обычной речи: «уборка (по договорённости)»
     * должна дойти до клиента целиком, а не исчезнуть из письма.
     */
    return value ?? match;
  };
  return text
    ...
    <Modal open onClose={onClose} title={template ? 'Правка шаблона КП' : 'Новый шаблон КП'} wide>
      <div className="space-y-3">
        ...
        {/*
          ...
          Раньше здесь был перечень с двойными фигурными скобками —
          «{{client}} — имя клиента».
        ...
        */}
        <div className="rounded-xl border border-navy-100 bg-navy-50/60 p-3 text-sm text-navy-700">
          ...
          <p className="mt-2 text-xs text-navy-600">
            ...
            выбрано — метка встанет в конец основного текста. Обычные скобки
            в тексте писать можно: «уборка (по договорённости)» дойдёт до
            клиента как есть — подставляются только названия из списка.
          </p>
        </div>
