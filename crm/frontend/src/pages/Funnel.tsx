        ...
        */}
        ...
        >
          {/*
            ...
            Высота на телефоне — по содержимому (решение владельца). Раньше
            доска занимала ровно остаток экрана, и карточки листались внутри
            колонки: последняя упиралась в полоску с точками и стрелками и
            показывалась обрезанной, а страница дальше не ехала. Теперь
            колонка растёт по числу карточек, прокручивается вся страница,
            как обычный список. Отступ снизу — под ту самую полоску: она
            «липнет» к низу экрана и в конце страницы ложится на этот отступ,
            а не на последнюю карточку. На большом экране высоту по-прежнему
            задаёт замер (style), отступ не нужен.
          */}
          <div
            ref={boardRef}
            style={boardHeight ? { height: boardHeight } : undefined}
            className="board-scroll flex snap-x snap-mandatory gap-3 pb-10 pr-4 sm:snap-none sm:gap-4 sm:pb-0 sm:pr-0"
          >
            {board.map((col) => {
              /*
                ...
                >
                  {(provided, snapshot) => (
                    <div
                      ...
                      /*
                       * На большом экране карточки листаются внутри колонки.
                       * Библиотека перетаскивания сама прокручивает этот блок,
                       * когда тянешь карточку к его краю, — переносить заказ
                       * между этапами по-прежнему можно.
                       *
                       * На телефоне внутренней прокрутки нет: колонка растёт
                       * по числу карточек, а листается вся страница (решение
                       * владельца). Перетаскивания на телефоне и так нет —
                       * этап меняют стрелками на карточке.
                       */
                      className={`min-h-0 flex-1 space-y-2.5 overflow-y-visible rounded-2xl p-1 transition-colors sm:overflow-y-auto ${
