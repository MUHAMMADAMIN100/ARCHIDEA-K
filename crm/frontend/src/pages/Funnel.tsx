...
export function Funnel() {
  ...
  const changeStage = async (orderId: string, newStage: FunnelStage) => {
    ...
    /*
     * Обратно из «Оплачено» в «К оплате» — нельзя.
     *
     * Оплаченный заказ, возвращённый на этот этап, снова считался долгом:
     * в шапке этапа появлялась задолженность по заказу, за который клиент
     * уже полностью рассчитался. Остальные переходы из «Оплачено» остаются —
     * ошибочно закрытый заказ должно быть можно вернуть в работу.
     */
    if (newStage === 'DONE') {
      const order = (data ?? [])
        .flatMap((c) => c.orders)
