...
export function Funnel() {
   ...
   */
  const [boardHeight, setBoardHeight] = useState<number | string>();
  ...
  useEffect(() => {
    ...
    const measure = () => {
      /*
       * Телефон. Высота — «экран минус то, что стоит над доской», и обе части
       * считаются честно.
       *
       * Раньше здесь стояла формула из класса: 100svh минус 7.5rem, то есть
       * ровно 120 пикселей на шапку и фильтры. На узком экране фильтры
       * занимают втрое больше — доска уходила под нижний край экрана, и её
       * последние карточки было не достать: колонка кончалась ниже видимой
       * части. Отсюда и пустота под доской, и «не листается вбок» — стрелки
      ...
      const next = Math.max(
        320,
        Math.round(window.innerHeight - top - padOf - 4),
      );
      // порог в 2px: без него наблюдатель размеров зациклился бы сам на себе
      setBoardHeight((prev) =>
        typeof prev === 'number' && Math.abs(prev - next) < 2 ? prev : next,
      );
    };
    measure();
    /*
        ...
        */}
        ...
        >
          {/*
            ...
            Высота доски — ровно остаток экрана. Карточки листаются внутри
            колонки, и страница под доской не растёт: иначе на коротком этапе
            под ним тянулась бы пустота во весь рост самой длинной колонки, а
            добраться до соседних статусов было бы нечем. Чтобы последняя
            карточка не пряталась под точками и стрелками, запас добавлен
            внутри самой колонки — см. её класс ниже.
          */}
          <div
            ref={boardRef}
                ...
                >
                  {(provided, snapshot) => (
                    <div
                       ...
                       *
                       * Запас снизу на телефоне (pb-12) — под полоску с
                       * точками этапов и стрелками: она лежит на нижнем крае
                       * доски, и без запаса последняя карточка упиралась в неё
                       * и показывалась обрезанной, сколько ни листай. Теперь
                       * список долистывается до конца, и она видна целиком.
                       * На большом экране полоски нет — там прежний отступ.
                       */
                      className={`min-h-0 flex-1 space-y-2.5 overflow-y-auto rounded-2xl p-1 pb-12 transition-colors sm:pb-1 ${
