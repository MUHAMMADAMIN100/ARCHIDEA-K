...
export function useLiveUpdates(enabled: boolean, userId?: string): void {
  useEffect(() => {
    ...
    const apply = (raw: Incoming, fromStream: boolean): void => {
      ...
      /*
       * СВОИ изменения тоже применяем.
       *
       * Раньше они пропускались: считалось, что свой экран уже обновился сам.
       * Для списка, в котором работаешь, это правда — но только для него.
       * Аналитику, дашборд, книгу доходов и ведомости не обновляет никто:
       * закрыл заказ — и свежие цифры видел только после перезагрузки
       * страницы, хотя чужие изменения прилетали исправно.
       *
       * Перезапрос при этом безопасен: пока изменение не дошло до сервера,
       * ответы на чтение не принимаются (см. readIsStale в hooks.ts), а
       * экраны с незавершённой операцией держат паузу через pollPaused.
       */
